...
def main():
    ...
    parser.add_argument('--config_path', type=str, default='./cfgs/SR_36_ddp_tri.py',
...
def main_worker(gpu, ngpus_per_node, cfg):
    ...
    if cfg.rank <= 0:
        ...
        logger.print(model)
    classifier1 = models.model_zoo.get_model(cfg.classify_model_name, in_features=512, out_features=cfg.num_class)
    # classifier2 = models.model_zoo.get_model(cfg.classify_model_name, in_features=512, out_features=cfg.num_class)
    classifier3 = models.model_zoo.get_model(cfg.classify_model_name, in_features=512, out_features=cfg.num_class)
    ...
    for epoch in range(cfg.start_epoch, cfg.max_epoch + 1):
        ...
        #train for one epoch
        train(train_loader, model, [classifier1, classifier3], [loss1, loss2], optimizer, epoch, cfg, writer, logger)
...
def train(train_loader, backbone, head, criterion, optimizer, epoch, cfg, writer, logger):
    ...
    for batch, (inputs, labels, classes, diff_location) in tqdm(enumerate(train_loader, 1)):# tqdm: 进度条
        ...
        up_labels = labels[classes==1]
        down_labels = labels[classes==0]

        outputs_down, original_logits_down = head[1](features[classes==0], down_labels)
        outputs_up, original_logits_up = head[0](features[classes==1], up_labels)
        # outputs_up_back, original_logits_up_back = head[2](features_up, labels)
        # outputs, original_logits = head[0](torch.cat([features_up, features_down], 1), labels)
        _t["forward_pass"].toc()
        # normal_num = max( (1-classes).sum(), 1)
        # loss = criterion(outputs, labels)
        loss2 = criterion[0](outputs_up, up_labels)


        loss3 = criterion[0](outputs_down, down_labels)
        loss4 = criterion[1](head[0].module.weight, head[1].module.weight, labels)
        # loss4 = criterion(log_softmax_up_mask, labels)
        ...
        # total_loss = loss2 + loss3 + (0.7/2)*loss4
        total_loss = loss2 + loss3 + 0.35*loss4
        ...
        prec1_down, prec5_down = accuracy(original_logits_down.data, down_labels, topk = (1, 5))
        prec1_up, prec5_up = accuracy(original_logits_up.data, up_labels, topk = (1, 5))
        if cfg.distributed:

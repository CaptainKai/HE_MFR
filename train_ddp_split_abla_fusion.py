...
def main():
    ...
    parser.add_argument('--config_path', type=str, default='./cfgs/SR_36_ddp_split_abla_fusion.py',
...
def main_worker(gpu, ngpus_per_node, cfg):
    ...
    classifier1 = models.model_zoo.get_model(cfg.classify_model_name, in_features=cfg.final_channel, out_features=cfg.num_class)
    classifier2 = models.model_zoo.get_model(cfg.classify_model_name, in_features=cfg.final_channel, out_features=cfg.num_class)
    classifier = models.model_zoo.get_model(cfg.classify_model_name, in_features=cfg.final_channel, out_features=cfg.num_class)
    fusion_mode = models.model_zoo.get_model(cfg.fusion_mode, d=cfg.fusion_d, c=cfg.fusion_c, active_mode=cfg.active_mode)
    model = models.model_zoo.get_model(cfg.backbone_model_name, input_size=[112, 112], fusion_loss=classifier, loss_down=classifier2, fusion_mode=fusion_mode)
    ...
    for epoch in range(cfg.start_epoch, cfg.max_epoch + 1):
        ...
        train(train_loader, model, [classifier1], [loss1], optimizer, epoch, cfg, writer, logger) # TODO
...
def train(train_loader, backbone, head, criterion, optimizer, epoch, cfg, writer, logger):
    ...
    for batch, (inputs, labels, classes, diff_location) in tqdm(enumerate(train_loader, 1)):# tqdm: 进度条
        ...
        feature_up, outputs, original_logits, outputs_down, original_logits_down = backbone(inputs, classes, labels)
        ...
        outputs_up, original_logits_up = head[0](feature_up, labels)
        ...
        loss3 = criterion[0](outputs_down, labels[classes<1])
        ...
        prec1_down, prec5_down = accuracy(original_logits_down.data, labels[classes<1], topk = (1, 5))
        ...
        losses3.update(loss3.data.item()/factor, labels[classes<1].size(0))

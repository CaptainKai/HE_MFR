...
def main_worker(gpu, ngpus_per_node, cfg):
    ...
    mapping_model = models.model_zoo.get_model(cfg.mapping_model_name, inchannel=256, outchannel=256, opt=torch.concat) # TODO
    if cfg.rank <= 0:
        logger.print([(x, cfg.__dict__[x]) for x in list(cfg.__dict__.keys()) if "__" not in x])
        logger.print(model)
    classifier1 = models.model_zoo.get_model(cfg.classify_model_name, in_features=256, out_features=cfg.num_class) # TODO
    ...
    for epoch in range(cfg.start_epoch, cfg.max_epoch + 1):
        ...
        train(train_loader, model, [classifier1], [loss1], optimizer, epoch, cfg, writer, logger)
...
def train(train_loader, backbone, head, criterion, optimizer, epoch, cfg, writer, logger):
    ...
    for batch, (inputs, labels, classes, diff_location) in tqdm(enumerate(train_loader, 1)):# tqdm: 进度条
        ...
        outputs, original_logits = head[0](features_up, features_down, labels, classes)
        _t["forward_pass"].toc()
        ...
        # loss3 = criterion[2](log_softmax_down, labels)*(classes.shape[0]**2) / (mask_num**2)

...
def main():
    ...
    parser.add_argument('--config_path', type=str, default='./cfgs/res_50_ddp_sphereface2.py',
...
def main_worker(gpu, ngpus_per_node, cfg):
    ...
    classifier = models.model_zoo.get_model(cfg.classify_model_name, in_features=512, out_features=cfg.num_class, gFunc=True)
    ...
    dataset_train = dataloader_lmdb.ImageList(lmdb_path=cfg.lmdb_path, max_reader=cfg.lmdb_workers,num=cfg.datanum, 
                        ...
                        # format_transform=train_transform, augu_paral=False, ldm68=True, augu_rate=1.0,shuffle=False)
    if cfg.distributed:
    ...
    for epoch in range(cfg.start_epoch, cfg.max_epoch + 1):
        ...
        #train for one epoch
        train(train_loader, model, classifier, optimizer, epoch, cfg, writer, logger)
        ...
        if cfg.rank % ngpus_per_node == 0 or cfg.rank==-1:
            if epoch%5==0 and epoch<=30 or epoch>30:
                ...
                model.save(cfg.save_path + 'backbone_' + str(epoch) + '_checkpoint.pth')
...
def train(train_loader, backbone, head, optimizer, epoch, cfg, writer, logger):
    ...
    # for batch, (inputs, labels, classes, diff_location) in tqdm(enumerate(train_loader, 1)):# tqdm: 进度条
        ...
        original_logits, loss = head(features, labels)
        _t["forward_pass"].toc()

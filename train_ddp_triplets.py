...
def main():
    ...
    parser.add_argument('--config_path', type=str, default='./cfgs/SR_36_ddp_triplet.py',
...
from models.backbones import TripletLoss
...
def main_worker(gpu, ngpus_per_node, cfg):
    ...
    optimizer = torch.optim.SGD([{'params': model.parameters(), 'weight_decay': cfg.weight_decay}],
                        lr=cfg.lr,
    ...
    if cfg.distributed:
        ...
        if cfg.gpu is not None:
            ...
            cfg.triplet_batch_size = int(cfg.triplet_batch_size / ngpus_per_node)
    ...
    dataset_train = dataloader_lmdb.TripletList(num_triplets=cfg.iterations_per_epoch * cfg.triplet_batch_size,lmdb_path=cfg.lmdb_path, max_reader=cfg.lmdb_workers,num=cfg.datanum,
                        id_num=cfg.num_class, epoch=cfg.start_epoch,
                        num_human_identities_per_batch=cfg.num_human_identities_per_batch,
                        id_dict_path=None if cfg.id_dict_path is None else cfg.id_dict_path,
                        training_triplets_path=None if cfg.training_triplets_path is None else cfg.training_triplets_path,
                        format_transform=train_transform, preproc=None)
                        # format_transform=train_transform, augu_paral=False, ldm68=True, augu_rate=0.5)
    if cfg.distributed:
        train_sampler = torch.utils.data.distributed.DistributedSampler(dataset_train)
    else:
        train_sampler = None
    train_loader = torch.utils.data.DataLoader(dataset_train, batch_size=cfg.triplet_batch_size, shuffle = (train_sampler is None), num_workers=cfg.num_workers, pin_memory=True, sampler=train_sampler, drop_last=True)
    ...
    for epoch in range(cfg.start_epoch, cfg.max_epoch + 1):
        ...
        #train for one epoch
        train(train_loader, model, None, None, optimizer, epoch, cfg, writer, logger)

        if cfg.rank % ngpus_per_node == 0 or cfg.rank==-1:
            if epoch%20==0 and epoch<=100 or epoch>100 and epoch%15:
        ...
        if cfg.distributed and ngpus_per_node>1:
            dist.barrier()
        dataset_train.reset_triplets(epoch+1)
        train_loader = torch.utils.data.DataLoader(dataset_train, batch_size=cfg.triplet_batch_size, shuffle = (train_sampler is None), num_workers=cfg.num_workers, pin_memory=True, sampler=train_sampler, drop_last=True)
...
def train(train_loader, backbone, head, criterion, optimizer, epoch, cfg, writer, logger):
    ...
    for batch, (batch_sample) in tqdm(enumerate(train_loader, 1)):# tqdm: 进度条
        ...
        all_imgs = all_imgs.cuda(cfg.gpu, non_blocking=True)
        _t["forward_pass"].tic()
        anc_embeddings, pos_embeddings, neg_embeddings, backbone = forward_pass(
            imgs=all_imgs,
            model=backbone,
        ...
        if cfg.use_semihard_negatives:
            ...
            first_condition = (neg_dists - pos_dists < cfg.margin).cpu().numpy().flatten()
            ...
            all = (neg_dists - pos_dists < cfg.margin).cpu().numpy().flatten()
            valid_triplets = np.where(all == 1)
        ...
        triplet_loss = TripletLoss(margin=cfg.margin).forward(
        ...
        if cfg.distributed:
            loss = reduce_tensor(triplet_loss.clone().detach_())
        ...
        losses.update(triplet_loss.data.item()/factor, num_valid_training_triplets)
        ...
        if ( ((batch + 1) % cfg.log_interval == 0) or batch == 0 ) and cfg.rank <= 0:
            ...
            logger.print('Epoch {}/{} Batch {}/{} eta: {}\t'
                            ...
                            .format(
                                epoch, cfg.max_epoch, batch + 1, len(train_loader), str(datetime.timedelta(seconds=eta)), loss = losses
                                # , top1 = top1, top5 = top5
                                ))
            logger.print("=" * 60)
            # sys.stdout.flush()
    ...
    if cfg.rank <= 0:
        logger.print('Epoch: {}/{} eta: {}\t''Training Loss {loss.val:.4f} ({loss.avg:.4f})\t'
                ...
                # 'Training Prec@5 {top5.val:.3f} ({top5.avg:.3f})'
                .format(
                    epoch, cfg.max_epoch, str(datetime.timedelta(seconds=eta)), loss = losses
                    # , top1 = top1, top5 = top5
                    ))
        logger.print("=" * 60)

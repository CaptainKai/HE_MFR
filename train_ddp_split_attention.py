...
def main():
    ...
    parser.add_argument('--config_path', type=str, default='./cfgs/res_50_ddp_split_attention.py',
...
def main_worker(gpu, ngpus_per_node, cfg):
    ...
    dataset_train = dataloader_lmdb.ImageList(lmdb_path=cfg.lmdb_path, max_reader=cfg.lmdb_workers,num=cfg.datanum, 
                        format_transform=train_transform,shuffle=False, augu_paral=False, ldm68=True, augu_rate=0.5)
    if cfg.distributed:
...
def train(train_loader, backbone, head, criterion, optimizer, epoch, cfg, writer, logger):
    ...
    _t['data_pass'].tic()
    _t['train_pass'].tic()
    for batch, (inputs, labels, classes) in tqdm(enumerate(train_loader, 1)):# tqdm: 进度条
        _t['data_pass'].toc()
        # compute output
        ...
        _t["forward_pass"].tic()
        features_up, features_down, features_up_mask = backbone(inputs)
        # features_up = torch.norm(features_up, 2, dim=1)
        # features_down = torch.norm(features_down, 2, dim=1)
        ...
        loss2 = criterion(features_up_mask, classes)

...
def main():
    ...
    parser.add_argument('--config_path', type=str, default='./cfgs/SR_36_ddp_split_abla_tri.py',
...
def main_worker(gpu, ngpus_per_node, cfg):
    ...
    if cfg.resume_net_classifier is not None:
        ...
        # state_dict = checkpoint['CLASSIFIER3']
        # new_state_dict = OrderedDict()
        # for k, v in state_dict.items():
        #     head = k[:7]
        #     if head == 'module.':
    ...
    dataset_train = dataloader_lmdb.ImageList(lmdb_path=cfg.lmdb_path, max_reader=cfg.lmdb_workers,num=cfg.datanum, 
                        # format_transform=train_transform, augu_paral=True, ldm68=True, augu_rate=1.0,shuffle=False)
    ...
    if cfg.rank <= 0:    
        logger.print('\n'.join(['%s:%s' % item if "indexlist" not in item else "indexlist: ignore" for item in dataset_train.__dict__.items()]))
    ...
    from models.backbones import CosineSimilarityLoss
    loss4 = CosineSimilarityLoss()
    ...
    for epoch in range(cfg.start_epoch, cfg.max_epoch + 1):
        ...
        #train for one epoch
        train(train_loader, model, [classifier1, classifier2], [loss1,loss2,loss3, loss4], optimizer, epoch, cfg, writer, logger)
...
def train(train_loader, backbone, head, criterion, optimizer, epoch, cfg, writer, logger):
    ...
    # for batch, (inputs, mask_inputs, labels, classes, diff_location) in tqdm(enumerate(train_loader, 1)):# tqdm: 进度条
        _t['data_pass'].toc()
        # compute output
        inputs = inputs.cuda(cfg.gpu, non_blocking=True)
        # mask_inputs = mask_inputs.cuda(cfg.gpu, non_blocking=True) # TODO
        ...
        # mask_outputs_up, mask_original_logits_up = head[2](mask_features_up, labels) # TODO
        # mask_outputs_up, mask_original_logits_up = head[2](features_up, labels)
        ...
        # loss2 = criterion[2](log_softmax_up, labels)*(classes.shape[0]**1) / (normal_num**1)
        ...
        # log_softmax_up_mask = criterion[1](outputs_up)*(classes.unsqueeze(1))
        # loss4 = criterion[2](log_softmax_up_mask, labels)*(classes.shape[0]**1) / (classes.shape[0]**1-normal_num**1)

        loss2 = criterion[0](outputs_up, labels)
        # log_softmax_up = criterion[1](outputs_up)*(1-classes.unsqueeze(1))
        # loss2 = criterion[2](log_softmax_up, labels)
        # loss3 = criterion[0](outputs_down, labels)
        # mask_num = max( (classes).sum(), 1)
        # log_softmax_up_mask = criterion[1](mask_outputs_up)*(classes.unsqueeze(1))
        # loss4 = criterion[2](log_softmax_up_mask, labels)*(classes.shape[0]**1) / mask_num
        # loss4 = criterion[2](log_softmax_up_mask, labels)
        loss5 = criterion[3](head[0].module.weight, head[1].module.weight, labels)
        # loss4 = criterion[0](mask_outputs_up, labels)
        ...
        # total_loss = 0.5*loss2 + loss3 + 0.5*loss4 # TODO
        # total_loss = loss2 + loss3 + loss4 + (0.7/2)*loss5 # 错误样本基本上是30*0.1的梯度，这里比较小，所以算是取0.35
        total_loss = loss2 + loss3 + loss5 # 错误样本基本上是30*0.1的梯度，这里比较小，所以算是取0.35
        ...
        if cfg.distributed:
            ...
            loss5 = reduce_tensor(loss5.clone().detach_())
        ...
        # losses4.update(loss4.data.item()/factor, inputs.size(0))
        losses5.update(loss5.data.item()/factor, inputs.size(0))
        ...
        if ( ((batch + 1) % cfg.log_interval == 0) or batch == 0 ) and cfg.rank <= 0:
            ...
            logger.print('Epoch {}/{} Batch {}/{} eta: {}\t'
                            ...
                            'Training Loss5 {loss5.val:.4f} ({loss5.avg:.4f})\t'
    ...
    if cfg.rank <= 0:
        ...
        writer.add_scalar("Training_Loss5", epoch_loss5, epoch)

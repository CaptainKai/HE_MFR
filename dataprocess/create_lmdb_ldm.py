...
parser.add_argument('--gt_paths', type=list, default=['/home/ubuntu/data1/lk/celebrity_msra_lmk/msra_lmk'],
...
parser.add_argument('--lmdb_path', type=str, default='/home/ubuntu/data4/lk/data/lmdb_ldm',
...
for gt_path in gt_paths:
    ...
    while gt:
        ...
        # print(new_p)
        # for i in range(5):
        #     cv2.circle(res, (int(new_p[i][0]),int(new_p[i][1])), 1, (255,0,0), 2)
        # cv2.imshow("aligned",res)
        # cv2.waitKey(0)
        # exit(0)
        if save:
        ...
        # masked_image, success = face_maker.mask(pic_frame, color_index)
        # if success:
        #     res = cv2.warpAffine(np.asarray(masked_image), M, (int(unit_size),int(unit_size)))
        #     image_key = "mask-image-%09d"%(cnt)
        #     label_key = "label-%09d"%(cnt)
        #     image_keys.append(image_key)
        #     label_keys.append(label_key)

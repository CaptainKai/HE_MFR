...
具体功能：
对原图进行对齐，并用对齐之后的68点对其佩戴口罩，仅存储佩戴完口罩的图片和5点关键点
'''
import argparse
...
parser.add_argument('--lmdb_path', type=str, default='/home/ubuntu/data4/lk/data/lmdb_mask_augu_pair',
...
for gt_path in gt_paths:
    ...
    while gt:
        ...
        point_68 = [float(x) for x in line[12:]]
        new_point_68=[]
        for i in range(68):
        ...
        if success:
            ...
            if save:
                dest_path = os.path.join(save_path,"mask_"+pic_name)
                if not os.path.exists(dest_path):
                    os.makedirs(dest_path)
                cv2.imwrite(dest_path, res)

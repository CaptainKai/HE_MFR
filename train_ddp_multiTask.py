...
def main():
    ...
    parser.add_argument('--config_path', type=str, default='./cfgs/am_36_ddp_multiTask.py',
...
def main_worker(gpu, ngpus_per_node, cfg):
    ...
    classifier2 = models.model_zoo.get_model(cfg.classify_model_name2, in_features=512, out_features=2)
    optimizer = torch.optim.SGD([{'params': model.parameters(), 'weight_decay': cfg.weight_decay}, {'params': classifier.parameters(), 'weight_decay': cfg.weight_decay}, {'params': classifier2.parameters(), 'weight_decay': cfg.weight_decay}],
                        lr=cfg.lr,
    ...
    for epoch in range(cfg.start_epoch, cfg.max_epoch + 1):
        ...
        #train for one epoch
        train(train_loader, model, [classifier, classifier2], loss, optimizer, epoch, cfg, writer, logger)
...
def train(train_loader, backbone, head, criterion, optimizer, epoch, cfg, writer, logger):
    ...
    for batch, (inputs, labels, classes, diff_location) in tqdm(enumerate(train_loader, 1)):# tqdm: 进度条
        ...
        _t["forward_pass"].tic()
        features, features_mask = backbone(inputs)
        outputs, original_logits = head[0](features, labels)
        outputs_mask= head[1](features_mask, labels)
        _t["forward_pass"].toc()
       
        loss1 = criterion(outputs, labels)
        loss2 = criterion(outputs_mask, classes)
        total_loss = loss1 + torch.log(loss2+1.0)
        ...
        if ( ((batch + 1) % cfg.log_interval == 0) or batch == 0 ) and cfg.rank <= 0:
            ...
            logger.print('Epoch {}/{} Batch {}/{} eta: {}\t'
                            ...
                            'Training Total_Loss {total_loss.val:.4f} ({total_loss.avg:.4f})\t'

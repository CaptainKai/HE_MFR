...
from functions.diverse_learning_loss import DiverseLearnLoss
'''
...
def main():
    ...
    parser.add_argument('--config_path', type=str, default='./cfgs/hpda_res50_11.py',
...
def main_worker(gpu, ngpus_per_node, cfg):
    ...
    if cfg.resume_net_model is not None:
        ...
        for k, v in state_dict.items():
            ...
            # namelist_nograd.append(name)
        model.load_state_dict(new_state_dict, strict=False)

        # for name, value in model.named_parameters(): # 固定参数
        #     # print('name: {0},\t grad: {1}'.format(name, value.requires_grad))
        #     if name in namelist_nograd and value.requires_grad:
        #         value.requires_grad = False # 停止gradient计算
    ...
    if cfg.resume_net_classifier is not None:
        ...
        # for name, value in classifier.named_parameters(): # 固定参数
        #     # print('name: {0},\t grad: {1}'.format(name, value.requires_grad))
        #     if name in namelist_nograd and value.requires_grad:
    ...
    loss2 = DiverseLearnLoss(batch_size=cfg.batch_size)
    ...
    for epoch in range(cfg.start_epoch, cfg.max_epoch + 1):
        ...
        #train for one epoch
        train(train_loader, model, classifier, [loss1, loss2], optimizer, epoch, cfg, writer, logger)

        if cfg.rank % ngpus_per_node == 0 or cfg.rank==-1:
            if epoch%5==0 and epoch<=30 or epoch>30:
...
def train(train_loader, backbone, head, criterion, optimizer, epoch, cfg, writer, logger):
    ...
    for batch, (inputs, labels) in tqdm(enumerate(train_loader, 1)):# tqdm: 进度条
        ...
        if cfg.distributed:
            ...
            loss1 = reduce_tensor(loss1.clone().detach_())
        ...
        losses.update(total_loss.data.item()/factor, inputs.size(0))
        losses1.update(loss1.data.item()/factor, inputs.size(0))
        ...
        if ( ((batch + 1) % cfg.log_interval == 0) or batch == 0 ) and cfg.rank <= 0:
            ...
            logger.print('Epoch {}/{} Batch {}/{} eta: {}\t'
                            ...
                            'Training Prec@5 {top5.val:.3f} ({top5.avg:.3f})\t'.format(
                                epoch, cfg.max_epoch, batch + 1, len(train_loader), str(datetime.timedelta(seconds=eta)), loss = losses, loss1 = losses1, loss2 = losses2,top1 = top1, top5 = top5))
            logger.print("=" * 60)
    ...
    if cfg.rank <= 0:
        logger.print('Epoch: {}/{} eta: {}\t'
                ...
                'Training Prec@1 {top1.val:.3f} ({top1.avg:.3f})\t'
                'Training Prec@5 {top5.val:.3f} ({top5.avg:.3f})'.format(
                    epoch, cfg.max_epoch, str(datetime.timedelta(seconds=eta)), loss = losses, loss1 = losses1, loss2 = losses2, top1 = top1, top5 = top5))
        logger.print("=" * 60)

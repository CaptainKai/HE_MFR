...
def main():
    ...
    parser.add_argument('--config_path', type=str, default='./cfgs/SR_36_ddp_split_abla.py',
...
def main_worker(gpu, ngpus_per_node, cfg):
    ...
    for epoch in range(cfg.start_epoch, cfg.max_epoch + 1):
        ...
        #train for one epoch
        train(train_loader, model, [classifier1, classifier2], [loss1], optimizer, epoch, cfg, writer, logger)
...
def train(train_loader, backbone, head, criterion, optimizer, epoch, cfg, writer, logger):
    ...
    for batch, (inputs, labels, classes, diff_location) in tqdm(enumerate(train_loader, 1)):# tqdm: 进度条
        ...
        outputs_down, original_logits_down = head[1](features_down[classes==0], down_labels)
        outputs_up, original_logits_up = head[0](features_up[classes==1], up_labels)

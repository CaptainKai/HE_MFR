...
对epoch的问题进行了修正
纯口罩数据训练的版本，主要是懒得改数据获取的参数，所以保存了一个这样的版本
'''

def main():
    parser = argparse.ArgumentParser(description='PyTorch amsoft training')
    parser.add_argument('--config_path', type=str, default='./cfgs/res_50_ddp.py',
...
def main_worker(gpu, ngpus_per_node, cfg):
    ...
    dataset_train = dataloader_lmdb.ImageList(lmdb_path=cfg.lmdb_path, max_reader=cfg.lmdb_workers,num=cfg.datanum, 
                        format_transform=train_transform,shuffle=False, ldm68=True, augu_rate=1)
    if cfg.distributed:
